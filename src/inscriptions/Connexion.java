...
public class Connexion {
	...
	public static void AjouterPersonne(String nom, String prenom, String mail)
	{
		...
		try
		{
			...
			if ( ajoutPers == 1) {
	        	...
	        	System.out.println("____Vous avez bien ajoutez une personne" );
	        }
	        else{
	        	System.out.println("____L'ajout de la personne n'a pas ete pris en compte____");
	        }
		}
		catch(Exception e)
		{
		...
		try
		{
			...
			statement = connexion.createStatement();
			 ...
			 int ModifPers = statement.executeUpdate("UPDATE personne SET pers_prenom = '"+ nouveauPrenom +"' WHERE pers_prenom = '"+ ancienPrenom +"'");
		        ...
		        if(ModifPers == 1)
		        {
		        	System.out.println("____La modification a bien ete prise en compte____" );
		        }
		        else{
		        	System.out.println("____La modification n'a PAS bien ete prise en compte____" );
		        }
		}
		catch(Exception e)
		{	
			...
			statement = connexion.createStatement();
			 ...
			 int ModifPers = statement.executeUpdate("UPDATE personne SET pers_mail = '"+ nouveauMail +"' WHERE pers_mail = '"+ ancienMail +"'");
		        ...
		        if(ModifPers == 1)
		        {
		        	...
		        	System.out.println("____La modification n'a PAS ete prise en compte____" );
		        }
		}
		catch(Exception e)
		{	
	...
	public void MofidierDateCloture(Date date, int idCompet)
	{
		try
		{
			 ...
			 int ModifDate = statement.executeUpdate( "UPDATE competition SET compet_dateCloture = "+ date +" WHERE id_compet = "+ idCompet +"" );
		        System.out.println( "____Requ�te d'affichage des candidats effectuee!____" );
	...
	public static void SupprimerCandidat(String nomCand)
	{
		...
		try
		{
			...
			if(verifCand(id) == 1)
			{
				...
				if(supprEquipe == 1)
				{
					System.out.println("____L'equipe a bien �t� supprimee____" );
				}
				else
				{
		        	System.out.println("____La suppression de l'equipe n'a PAS  ete prise en compte____" );
		        }
			}
			else
			{
				...
				if(supprPers == 1)
				{
					System.out.println("____La personne a bien ete supprimee____" );
				}
				else
				{
		        	System.out.println("____La suppression de la personne n'a PAS  ete prise en compte____" );
		        }
			}
		...
		try
		{
			...
			if(verifCandCompet.next())
			{
				 ...
				 while(VerifNomCompet.next())
			     {
					 ...
					 System.out.println("____le candidat n�"+idCand+" est deja inscrit a la competition n�"+idCompet+" - "+ VerifNomCompet.getString("compet_nom") );
			     }
			...
			else
			{
				...
				if(cand == 1)
				{
					...
					if(verifEnEquipe.next())
					{
						...
						{
							System.out.println("____Vous vous etes bien inscrit pour participer a la competition n�"+idCompet+"");
						}
						else
						{
				        	System.out.println("____L'inscription n'a pas ete prise en compte____" );
				        }
					}
					else
					{
						System.out.println("____Vous ne pouvez pas vous inscrire car cette competition se d�roule en individuelle");
					}
				}
				else
				{
					...
					{
						System.out.println("____Vous ne pouvez pas vous inscrire car cette competition se joue en equipe");
					}
				}
	...
	public static void inscriptionPersonneDansEquipe(int idPers, int idEquipe)
	{
		try
		{
			...
			{
				System.out.println("____Vous etes deja inscrit dans l'equipe n "+idEquipe+"");
			}
			else
			{
	...
	public static void SupprimerCompetition(String NomCompet)
	{
		...
		try
		{
		        ...
		        if(SupprCompet == 1){
		        	System.out.println("____La suppression a bien ete prise en compte____" );
		        }
		        else{
		        	System.out.println("____La suppression n'a PAS �t� prise en compte____" );
		        }
		}
		catch(Exception e)
		{	
